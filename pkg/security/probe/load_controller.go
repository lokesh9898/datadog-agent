...
func (lc *LoadController) discardNoisiestProcess() {
	...
	log.Debugf("discarding %s events from pid %d for %s seconds", maxKey.Event, maxKey.Pid, lc.DiscarderTimeout)
	if _, err := discardPIDWithTimeout(lc.probe, maxKey.Event, maxKey.Pid, lc.DiscarderTimeout); err != nil {
		log.Debugf("couldn't insert temporary discarder: %v", err)
		return
	}
	...
	if err := lc.statsdClient.Count(MetricPrefix+".load_controller.pids_discarder", 1, tags, 1.0); err != nil {
		log.Debugf("couldn't send load_controller.pids_discarder metric: %v", err)
		return
	}
}
...
func (lc *LoadController) Start(ctx context.Context) {
	...
	log.Debugf("load controller started")
	for {
		select {
		case <-ticker.C:
			lc.cleanup()
